...
for t, message in message_types.groupby('message_type'):
    message_fields[t] = namedtuple(
        typename=t, field_names=message.name.tolist())
...
with file_name.open('rb') as data:
    while True:
        ...
        message_size = int.from_bytes(
            data.read(2), byteorder='big', signed=False)
        ...
        if message_type == 'S':
            ...
            print('\n', event_codes.get(
                message.event_code.decode('ascii'), 'Error'))
...
counter = counter[['Message Type', '# Trades']
...
with pd.HDFStore(itch_store) as store:
    ...
    trades = store['P'].append(store['Q'].rename(
        columns={'cross_price': 'price'}), sort=False).merge(stocks)

trades['value'] = trades.shares.mul(trades.price)
trades['value_share'] = trades.value.div(trades.value.sum())

trade_summary = trades.groupby(
    'stock').value_share.sum().sort_values(ascending=False)
